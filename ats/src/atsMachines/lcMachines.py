        ...
        for killTimes in xrange(0,1):
            ...
            for line in self.lastSqueueResult:
                if test.jobname in line:
                    scancelCommand = f'scancel {line.split()[0]}'
                    if debug():
                        log(f"---- LCMachineCore::kill: {scancelCommand}", echo=True)
            ...
            if debug():
                log(
                    f"---- LCMachineCore::kill, CALLED AGAIN {killTimes + 1} test name: {test.jobname} {test.serialNumber}",
                    echo=True,
                )
